...
const HeroSection = () => {
  ...
  return (
    <section ref={sectionRef} className="min-h-screen relative overflow-hidden grid-pattern">
      {/* Grid Lines Overlay */}
        ...
        <div ref={partnersRef} className="mt-16 border-t border-foreground pt-8">
          ...
          <div className="flex flex-wrap items-center gap-4">
            {["Chainlink", "Tron", "BNB", "OKX", "Certik", "Polygon"].map((partner) => (
              <div
                key={partner}
                className="partner-item px-4 py-2 border border-foreground/30 hover:border-foreground hover:bg-foreground hover:text-background transition-all duration-300 font-mono text-sm cursor-pointer"
              >
                {partner}
              </div>
            ))}
